...
function dkpSlideTexts(jsData) {
  const controls = jsData?.controls || {};
  const texts = [];
  for (const ctl of Object.values(controls)) {
    if (!ctl.text) continue;
    const t = stripHtmlToText(ctl.text);
...
function dkpBubble(texts) {
  ...
  return { bgColor: '#EFF6FF', segments };
}
...
export async function extractDkpTopics(file, allowedIds = null, onProgress = null) {
  const zip  = await JSZip.loadAsync(file);
  const { caption: bookCaption, refs: allRefs } = await dkpBookInfo(zip);
  ...
  for (const ref of refs) {
    ...
    if (jsEntry) {
      try {
        const jsData = JSON.parse((await jsEntry.async('string')).replace(/^\uFEFF/, ''));
        bubble = dkpBubble(dkpSlideTexts(jsData));
      } catch { /* skip */ }
    }

    const screenW = 1024, screenH = 672;
    topics.push({
      title: ref.title,
      intro: null,
      screenW,
